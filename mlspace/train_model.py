...
class TrainModel():
    ...
    def train(self):
        ...
        self.X_train, self.Y_train = X[trainset], Y[trainset]

        # 2) Y value are null ( for test )
        self.X_test, self.Y_test = X[~trainset], Y[~trainset]
        ...
        cv_split = self.X_train.index < '2012-05-27'
        X_train_cv, Y_train_cv = self.X_train[cv_split], self.Y_train[cv_split]
        X_val_cv, Y_val_cv = self.X_train[~cv_split], self.Y_train[~cv_split]

        logging.info("{0} {1}".format(self.X_train.shape, self.Y_train.shape))
        ...
        for feature, importance in sorted(zip(self.model.feature_importances_, self.X_train.columns), key=lambda x: x[0], reverse=True):
    ...
    def store(self):
        """
        Function to store the predicted data and the models
        """
        ...
        np.save(FULL_TEST_Y_HAT, self.Y_test_hat)
        np.save(FULL_TEST_Y, self.Y_test)
